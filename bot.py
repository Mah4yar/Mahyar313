...
def get_help1(text,chat,bot):                                
    text = open('help1.txt','r').read()
    if chat['abs_object']['type'] == 'Group':
        bot.sendMessage(chat['object_guid'], 'نتایج کامل به پیوی شما ارسال شد', chat['last_message']['message_id'])
        bot.sendMessage(chat['last_message']['author_object_guid'], text)                                        
    elif chat['abs_object']['type'] == 'User':
...
def get_help3(text,chat,bot):                                
    text = open('help3.txt','r').read()
    if chat['abs_object']['type'] == 'Group':
        bot.sendMessage(chat['object_guid'], 'نتایج کامل به پیوی شما ارسال شد', chat['last_message']['message_id'])
        bot.sendMessage(chat['last_message']['author_object_guid'], text)                                        
    elif chat['abs_object']['type'] == 'User':
...
g_usvl = ''
test_usvl = ''
auth = "yumdxrifmbxvcesvmfwiyhhtfarcfajx"
bot = Bot(auth)
list_message_seened = []
                    ...
                    if 'SendMessages' in access and chat['last_message']['type'] == 'Text' and text.strip() != '':
                        ...
                        if not m_id in list_message_seened:
                            ...
                            if text == '!start':
                                ...
                                tawd39 = Thread(target=get_help1, args=(text, chat, bot,))
                                tawd39.start()
                            elif text.startswith('/Tools'):
                                tawd40 = Thread(target=get_help2, args=(text, chat, bot,))
                                tawd40.start()
                            elif text.startswith('/google'):
                                tawd41 = Thread(target=get_help3, args=(text, chat, bot,))
                                tawd41.start()
